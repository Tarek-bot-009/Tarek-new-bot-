...
module.exports.run = async function ({ api, event, args, Users }) {
  try {
    ...
    if (args[0] === 'edit') {
      ...
      if (command.length < 2) {
        return api.sendMessage('âŒ | Invalid format! Use edit [YourMessage] - [NewReply]', event.threadID, event.messageID);
      }
    ...
    if (args[0] === 'teach' && args[1] !== 'amar' && args[1] !== 'react') {
      ...
      if (command.length < 2) {
        return api.sendMessage('âŒ | Invalid format! Use [YourMessage] - [Reply1], [Reply2], [Reply3]... OR remove [YourMessage] OR list OR edit [YourMessage] - [NewReply]', event.threadID, event.messageID);
      }
    ...
    if (args[0] === 'teach' && args[1] === 'react') {
      ...
      if (command.length < 2) {
        return api.sendMessage('âŒ | Invalid format! Use [teach] [YourMessage] - [Reply1], [Reply2], [Reply3]... OR [teach] [react] [YourMessage] - [react1], [react2], [react3]... OR remove [YourMessage] OR list OR edit [YourMessage] - [NewReply]', event.threadID, event.messageID);
      }
...
try{
    ...
    if(body.startsWith("baby") || body.startsWith("bby") || body.startsWith("janu")){
      ...
      if(!arr) return message.reply("Yes ðŸ˜€, i am here ")
